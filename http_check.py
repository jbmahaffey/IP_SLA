...
import os, re, time, requests, signal, subprocess
...
def Failure():
    ...
    #subprocess.check_output('FastCli /mnt/flash/%s' % (failscript), shell=True)
    return 1

def Failback():
    print('failback')
    #primaryscript='primary_commands'
    #subprocess.check_output('FastCli /mnt/flash/%s' % (primaryscript), shell=True)
    return 0    
